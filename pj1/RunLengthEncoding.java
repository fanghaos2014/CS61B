...
private DListNode1 node;
  ...
  public RunLengthEncoding(int width, int height) {
    ...
    this.node=new DListNode1(a);
    this.dlist.head=this.node;
  }
  ...
  public RunLengthEncoding(int width, int height, int[] red, int[] green,
    ...
    this.height=height;
    this.dlist=new DList1();
    this.dlist.head=new DListNode1();
    for (int a=red.length-1; a>=0; a--) {
      int node_data[]=new int[] {runLengths[a], red[a], green[a], blue[a]};
      this.dlist.insertFront(node_data);
      System.out.println(dlist.head.item[0]);
    }
  }
...
return this.width;
  ...
  public RunIterator iterator() {
    ...
    RunIterator iterate =new RunIterator(this.dlist);
    return iterate;
  }
  ...
  public PixImage toPixImage() {
    // RunLengthEncoding dummy=new RunLengthEncoding(width,height);
    
    RunIterator a1=this.iterator();
    System.out.println("A1 null " + a1 == null);
    int count=0;
    int length=0;
    PixImage pic = new PixImage(this.width,this.height);

    while (a1.hasNext() == true) {
      int arrayData[] = a1.next();
      for (int j = 0; j < arrayData[0]; j++) {
        pic.setPixel(length,count, (short) arrayData[1], (short) arrayData[2], (short) arrayData[3]);
        length++;
        if (length == width) {
          count++;
          length=0;
