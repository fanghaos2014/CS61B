...
public class SList {
  ...
  public void squish() {
    ...
    while (tail!=null) {
    if (nth(position).equals(nth(position+1))) {
  ...
  public void twin() {
    ...
    for (int position=1; position<this.length()+1; position++) {
      if (dummy.next==null) {
        SListNode node=new SListNode(nth(2*position-1),null);
        dummy.next=node;
      } else {

    SListNode node=new SListNode(nth(2*position-1),dummy.next);
    dummy.next=node;
    dummy=node.next;
  }
